...
在 `keep_rules` / `delete_rules` / `whitelist_rules` 中，规则系统通过模式末尾是否带 `/` 来区分匹配目标——`"xxx/"` 匹配目录自身，`"xxx/*"` 匹配目录下的文件。虽然两个模式的匹配对象不同（一个匹配目录、一个匹配文件），但它们的**作用域是一致的**：都是在描述"对 `xxx/` 这个区域内的内容做什么"。用户按直觉配置通常不会出问题。

但如果让 `rotate_rules` 也支持匹配目录，`"xxx/"` 和 `"xxx/*"` 这两个外形相似的配置将产生完全不可互换的语义：

- `rotate_rules.count."xxx/*" = 5` → 限制 `xxx/` 下的**文件数量**不超过 5 个（合理、直观）
- `rotate_rules.count."xxx/" = 5` → 「目录数量不超过 5 个」还是「把目录当作一个文件来计数」？（无论哪种解读，都和 `"xxx/*"` 的行为截然不同）
- `rotate_rules.size."xxx/" = "1GB"` → 是目录自身元数据的体积，还是目录下所有文件的递归总体积？（两种解读都有合理性，答案不是唯一的）

即便不考虑误配置的问题，给 `rotate_rules` 设计目录匹配逻辑也极为困难。轮转模式的操作单元是**文件**：遍历文件列表、按 `mtime` 排序、逐个移除旧文件以腾出空间。如果引入目录级别的分组，会引发一系列难以自洽的问题：
...
- **移除语义模糊**：轮转一个文件是指移动或删除它，那轮转一个目录是指删除整个目录，还是只删除目录中的部分旧文件？
